...
CREATE TABLE game (gamenumber STRING PRIMARY KEY,status STRING,starter STRING,tokenplayer STRING,deckontable STRING,restcards STRING,winner STRING, mysterycard INTEGER);
...
CREATE TABLE gameuser (player STRING,game STRING,status STRING,cards STRING,ordernr INTEGER,PRIMARY KEY (player, game));
  CREATE TABLE player (ip STRING PRIMARY KEY,id STRING,name STRING,status STRING, password STRING, datafield STRING);
